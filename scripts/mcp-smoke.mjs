...
try {
  ...
  assert.ok(tools.tools.some((tool) => tool.name === "route_and_start_harness_task"));
  assert.ok(tools.tools.some((tool) => tool.name === "get_harness_task"));
  ...
  process.stdout.write("mcp smoke: 18 tools registered and legacy proxy flow passed over stdio\n");
} finally {
  await client.close();
  await harness.close();
}
