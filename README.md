...
Native delegation defaults to 120 seconds for small or mechanical work and 300 seconds for repository-scale read-only review, debugging, and general tasks; callers may set an explicit timeout up to 600 seconds. Read-only execution automatically walks the persisted ranked native-model chain after allowlisted timeouts, network failures, rate limits, overloads, upstream 5xx responses, or a candidate-specific missing model. Every candidate is revalidated against the live catalog, user-agent rankings are never silently invoked, and the current host stays reserved as the final fallback.

Each result includes `attemptChain` with candidate, effort, outcome, latency, error class/disposition, partial-write state, and any child session ID. Privacy-safe attempt and cooldown metadata is persisted without child output or session IDs. Workspace-write fallback is off by default and requires `allowWriteFallback: true`; it stops immediately if repository signals detect any write. `resumeSessionId` continues an OpenCode or Claude Code trajectory using the resume mechanisms confirmed by their installed CLI help. Codex resume remains disabled because its resume help does not expose the sandbox controls required by this adapter. Feedback is an explicit, inspectable prior; there is no opaque training in v0.1.
